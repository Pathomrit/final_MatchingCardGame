...
class _Easy extends State<Easy> {
  ...
  Future<List<Map<String, dynamic>>> fetchRandomData() async {
    ...
    final data = await getAppData("easy");
  ...
  void shuffleCard() {
    fetchRandomData().then((data) async {
      ...
      data.forEach((item) {
        ...
        // var file = File("${dir.path}/assets/AppData/${item['picImages']}");
        // final picBytes = file.readAsBytesSync();

        // Uint8List picBytes = base64Decode(item['picImages']);
        // String picImageData = base64Encode(picBytes);
        fetchedPicImages.add("${dir.path}/assets/AppData/${item['picImages']}");

        // var wordFile = File("${dir.path}/assets/AppData/${item['wordImages']}");
        // final wordBytes = wordFile.readAsBytesSync();
